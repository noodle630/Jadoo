...
router.get('/feeds/:feedId/progress', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { feedId } = req.params;
    let job = processingJobs.get(feedId);
    if (!job) {
        // Try BullMQ
        try {
            const jobs = yield feedQueue.getJobs(['active', 'waiting', 'completed', 'failed']);
            const bullJob = jobs.find((j) => j.data.feedId === feedId);
            if (bullJob) {
                job = {
                    feed_id: feedId,
...
function processFileInBackground(feedId, fileBuffer, fields) {
    return __awaiter(this, void 0, void 0, function* () {
        const updateProgress = (progress, message, data = {}) => {
            const jobData = Object.assign({ feed_id: feedId, status: progress < 100 ? 'processing' : 'completed', progress,
        ...
        try {
            ...
            try {
                ...
                transformResult = yield new Promise((resolve, reject) => {
                ...
                throw new Error('Transformer failed: ' + (err && err.error ? err.error : err));
            }
            ...
            const { data: uploadData, error: uploadError } = yield supabase.storage.from('feeds').upload(`${feedId}.xlsx`, xlsxBuffer, { contentType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', upsert: true });
            if (uploadError)
            ...
            const { error: dbError } = yield supabase.from('feeds').upsert({
                id: feedId,
            ...
            if (dbError)
                throw new Error('Supabase DB upsert failed: ' + dbError.message);
            // Final completion
            ...
            updateProgress(0, 'Processing failed', { error: error.message });
            // Log error to Supabase
            yield supabase.from('feeds').upsert({
                id: feedId,
                filename: fields.fileName || fields.filename || `${feedId}.xlsx`,
                platform: fields.platform,
                status: 'failed',
...
router.post('/simple-upload', upload.single('file'), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        yield feedQueue.add('feed-transform', {
            feedId,
...
router.get("/logs/:id", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const { data, error } = yield supabase.from("logs").select("*").eq("feed_id", id).order("row_number");
    if (error) {
        console.error(error);
...
router.get("/download/:file", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
...
router.get('/platforms/:platform/categories', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
...
router.get('/feeds/:feedId/download', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { feedId } = req.params;
    // Try to get the feed from Supabase
    const { data, error } = yield supabase.from('feeds').select('*').eq('id', feedId).single();
    if (error || !data) {
...
router.get('/feeds/:feedId/ready', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { feedId } = req.params;
    // Try to get the feed from Supabase
    const { data, error } = yield supabase.from('feeds').select('*').eq('id', feedId).single();
    ...
    try {
        ...
        const headResp = yield fetch(data.output_path, { method: 'HEAD' });
        if (headResp.ok) {
...
router.get('/jobs/:feedId/status', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { feedId } = req.params;
    try {
        // Get all jobs for this feed
        const jobs = yield feedQueue.getJobs(['active', 'waiting', 'completed', 'failed']);
        const job = jobs.find((j) => j.data.feedId === feedId);
        if (!job) {
        ...
        if (job.finishedOn && !job.failedReason) {
            try {
                const { data: feedData, error } = yield supabase
                    .from('feeds')
                    .select('*')
                    .eq('id', feedId)
                    .single();
...
// Analytics endpoints
router.get('/api/analytics/dashboard', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const dashboardData = yield analyticsService.getDashboardData();
        res.json({
            success: true,
            data: dashboardData
        });
...
router.get('/api/analytics/jobs', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { platform, category } = req.query;
        const jobSummary = yield analyticsService.getJobPerformanceSummary(platform, category);
        res.json({
            success: true,
            data: jobSummary
        });
...
router.get('/api/analytics/fields', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const fieldSummary = yield analyticsService.getFieldPerformanceSummary();
        res.json({
            success: true,
            data: fieldSummary
        });
...
router.get('/api/analytics/llm', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const llmSummary = yield analyticsService.getLLMPerformanceSummary();
        res.json({
            success: true,
            data: llmSummary
        });
...
router.get('/api/analytics/recent', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { limit = '10' } = req.query;
        const recentJobs = yield analyticsService.getRecentJobAnalytics(parseInt(limit));
        res.json({
            success: true,
            data: recentJobs
        });
