...
Response (JSON only, no explanation, no markdown, no code block):`;
        ...
        outputRows.forEach((row, i) => {
            outputColumns.forEach((col, j) => {
                const cell = worksheet.getRow(i + 6).getCell(j + 1);
                cell.value = row[col] || "";
                // Highlight errored cells
                if (row.status === 'ERROR' || (row.error && row.error.includes(col))) {
                    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFF0000' } };
                    cell.note = 'Error in this cell';
                }
            });
            // Highlight entire row if errored
            if (row.status === 'ERROR') {
                worksheet.getRow(i + 6).eachCell((cell) => {
        ...
        result.summary_json.error_rows = errorRows.map(r => ({ row: r.row_number, error: r.error_message }));
        ...
        if (!uploadSuccess) {
            ...
            return res.status(500).json({ error: 'Output file not available after processing', feed_id: id });
        }
