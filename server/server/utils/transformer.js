...
export const handleProcess = async (req = {}) => {
    ...
    try {
        ...
        outputColumns.forEach((col, idx) => {
            ...
            if (foundIdx === -1) {
                ...
                normalizedInputHeaders.forEach((h, i) => {
                    const score = stringSimilarity.compareTwoStrings(normCol, h);
                    if (score > bestScore) {
                        bestScore = score;
                        bestIdx = i;
                    }
        ...
        // 2. For each row, map and enrich as needed (strict)
        const outputRows = [];
        let totalLLMCalls = 0;
        let totalMapped = 0;
        let totalEnriched = 0;
        let totalErrors = 0;
        async function enrichField(row, col, tier, fieldDef) {
            return await enrichKeyField(row, col, '', tier);
        }
        for (const [rowIdx, rowRaw] of rows.entries()) {
            ...
            for (const col of outputColumns) {
                let value = '';
                let mapped = false;
                // Strict mapping: only map if headerMapping exists and value is non-blank, non-null, and length >= 2
                ...
                // If not mapped or value is blank/low quality, enrich
                if (!mapped) {
                    try {
                        value = await enrichField(row, col, tier, fieldDefinitions[col] || {});
                        totalLLMCalls++;
                        totalEnriched++;
                        logger.info({ message: '[ENRICH] LLM enrichment', rowIdx, col, value });
                    }
                    catch (e) {
                        logger.error({ message: '[ENRICH] LLM error', rowIdx, col, error: e });
                        totalErrors++;
                    }
                }
                outputRow[col] = value;
        ...
        logger.info({ message: '[SUMMARY] Strict mapping/enrichment complete', totalRows: outputRows.length, totalMapped, totalEnriched, totalLLMCalls, totalErrors });
        logger.info('[OUTPUT] Number of rows written to output', { outputRowCount: outputRows.length });
