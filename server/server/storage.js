export class MemStorage {
    ...
    feeds;
    templates;
    githubTokens; // userId -> token
    githubRepos; // feedId -> repo info
    userId;
    feedId;
    templateId;
    constructor() {
    ...
    async createFeed(feed) {
        ...
        const newFeed = { ...feed, id, name: feedName, processedAt: now, sourceDetails: feed.sourceDetails ?? {}, aiChanges: feed.aiChanges ?? {}, outputUrl: feed.outputUrl ?? null, itemCount: feed.itemCount ?? null };
        this.feeds.set(id, newFeed);
        return newFeed;
    }
    async updateFeed(id, feedUpdate) {
    ...
    async storeGithubToken(userId, token) {
    ...
    async storeGoogleToken(userId, token) {
        const user = await this.getUser(userId);
        if (!user) {
            return false;
        }
    ...
    async linkRepoToFeed(feedId, repoInfo) {
