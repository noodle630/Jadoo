...
import { transformRowWithGPT } from './transformer';
...
async function main() {
  ...
  const filledRow = templateHeaders.map((field: string) => transformedRow[field] ?? '');
  ...
  console.log('🔍 Missing fields:', templateHeaders.filter(f => !transformedRow[f]));
}
