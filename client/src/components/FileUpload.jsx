...
export default function FileUpload({ onFileAccepted, acceptedFileTypes = ['.csv', '.xlsx', '.xls'], maxFileSize = 10 * 1024 * 1024, // 10MB
allowMultiple = false, status = 'idle', progress = 0, error = '', }) {
    ...
    const fileRejectionItems = fileRejections.map(({ file, errors }) => (<Alert key={file.name} variant="destructive" className="mt-2 bg-red-900/20 border-red-800">
    ...
    return (<div className="w-full">
      {selectedFile && status !== 'error' ? (<Card className="p-4 border-gray-700 bg-gray-800">
          <div className="flex justify-between items-center mb-2">
            ...
            {status === 'idle' && (<Button variant="ghost" size="sm" onClick={removeFile} className="text-gray-400 hover:text-white">
                <X className="h-4 w-4"/>
              </Button>)}
          ...
          <div {...getRootProps()} className={`flex flex-col items-center justify-center border-2 border-dashed rounded-lg p-8 cursor-pointer transition-colors ${isDragActive
          ...
          {status === 'error' && error && (<Alert variant="destructive" className="mt-4 bg-red-900/20 border-red-800">
              <AlertCircle className="h-4 w-4"/>
              <AlertDescription className="text-sm">{error}</AlertDescription>
            </Alert>)}

          {fileRejectionItems.length > 0 && (<div className="mt-4">{fileRejectionItems}</div>)}
        </>)}
    </div>);
}
