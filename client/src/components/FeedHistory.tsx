...
export default function FeedHistory() {
  ...
  const filteredFeeds = feeds.filter(feed => {
    ...
    if (activeTab === 'amazon') return feed.marketplace === 'amazon';
    if (activeTab === 'walmart') return feed.marketplace === 'walmart';
    if (activeTab === 'others') {
  ...
  const getMarketplaceBadge = (marketplace: string) => {
    const marketplaceColors: Record<string, { bg: string, text: string, border: string, darkBg: string, darkText: string, darkBorder: string }> = {
      amazon: { 
        bg: 'bg-orange-50', 
    ...
    return (
      <Badge 
        variant="outline" 
        className={`${styles.bg} ${styles.text} ${styles.border} ${styles.darkBg} ${styles.darkText} ${styles.darkBorder} uppercase text-xs font-medium py-0.5`}
      >
        {marketplace}
      </Badge>
    );
            ...
            {filteredFeeds.length === 0 ? (
              ...
              <div className="divide-y divide-slate-200 dark:divide-slate-800">
                {filteredFeeds.map((feed) => (
                  <div key={feed.id} className="flex flex-col sm:flex-row sm:items-center justify-between py-4 gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center text-xs text-slate-500 dark:text-slate-400">
                        ...
                        {feed.itemCount && (
                          <span>{feed.itemCount.toLocaleString()} products</span>
                        )}
                      </div>
                    </div>
