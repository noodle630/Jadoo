...
export default function Sidebar({ isOpen, onClose }) {
    ...
    return (<aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col bg-slate-950 text-white transform transition-transform duration-300 ease-in-out md:translate-x-0 shadow-2xl shadow-slate-900/50 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`} aria-hidden={!isOpen && window.innerWidth < 768}>
      {/* Logo */}
      ...
      <nav className="flex-1 overflow-y-auto p-2">
        <ul className="space-y-1 px-2">
          ...
          <li>
            <Link href="/">
              <div className={`flex items-center gap-3 rounded-md px-3 py-2 text-sm transition-colors ${isActive("/")
            ? "bg-slate-800 text-white"
            : "text-slate-400 hover:bg-slate-900 hover:text-white"}`} onClick={handleLinkClick}>
                <Home size={18}/>
                <span>Dashboard</span>
              </div>
            </Link>
          ...
          <li className="mt-6">
            <Link href="/create-feed">
              <div className={`flex items-center gap-3 rounded-md px-3 py-2 text-sm transition-colors ${isActive("/create-feed")
