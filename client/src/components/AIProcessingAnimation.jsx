...
export default function AIProcessingAnimation({ step = 1, maxSteps = 4, insights = [], onComplete }) {
    ...
    return (<div className="relative w-full max-w-lg mx-auto h-44 overflow-hidden rounded-xl bg-gradient-to-br from-slate-950 to-slate-900 border border-slate-800">
      {/* Subtle background gradient */}
      <div className="absolute inset-0 overflow-hidden z-0">
        <div className="absolute inset-0 bg-gradient-to-br from-blue-500/5 to-indigo-500/5"></div>
        <motion.div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-blue-400/20 via-indigo-500/20 to-purple-500/20" animate={{
            opacity: [0.2, 0.5, 0.2]
        }} transition={{
            duration: 3,
            repeat: Infinity,
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center p-6">        
        ...
        <div className="h-6 flex items-center justify-center mb-5">
          <motion.p key={currentInsight} // Force animation to restart when text changes
     initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-sm text-slate-300 text-center">
        ...
        <div className="w-full max-w-sm relative">
          <div className="relative h-2 w-full bg-slate-800/70 rounded-full overflow-hidden">
            {/* Progress fill */}
            <motion.div className="h-full bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full relative overflow-hidden" initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.5, ease: "easeInOut" }}>
              {/* Shimmer effect inside progress */}
              <motion.div className="absolute top-0 bottom-0 w-20 bg-gradient-to-r from-transparent via-white/10 to-transparent" style={{ left: "-100%" }} animate={{ left: "100%" }} transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }}/>
            </motion.div>
          </div>
