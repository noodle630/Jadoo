...
export default function NewFeedV2() {
  ...
  const [selectedRow, setSelectedRow] = useState<TransformRow | null>(null);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const result: TransformResult = procRes.data;
            ...
            <ul className="space-y-1">
              {navItems.map((item) => (
                <li key={item.name}>
                  <a
                    href={item.href}
                    className="flex items-center px-6 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-700 rounded-lg transition-colors font-medium"
                  >
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 max-h-96 overflow-y-auto">
                <ul className="space-y-2">
                  {log.map((line, i) => (
                    <li key={i} className="text-sm font-mono">{line}</li>
                  ))}
                </ul>
              </div>
            </div>
          ...
          {status === "error" && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-6">
              ...
              <div className="bg-red-100 rounded-lg p-4 max-h-96 overflow-y-auto">
                <ul className="space-y-2">
                  {log.map((line, i) => (
                    <li key={i} className="text-sm font-mono text-red-800">{line}</li>
                  ))}
                </ul>
              </div>
              <button
