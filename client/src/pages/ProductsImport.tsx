...
export default function ProductsImport() {
  ...
  const [uploadStep, setUploadStep] = useState<'idle' | 'uploading' | 'processing' | 'completed' | 'error'>('idle');
    ...
    switch(uploadStep) {
      ...
      default:
        return (
          <div
            {...getRootProps()}
            className={`border-2 border-dashed rounded-lg p-12 text-center transition-colors cursor-pointer hover:bg-muted/50
