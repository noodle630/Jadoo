...
export default function FeedHistoryDebug() {
    ...
    const [inspectFeed, setInspectFeed] = useState(null);
        ...
        <div className="bg-slate-900 p-4 rounded-lg">
          ...
          <div className="mb-6">
            ...
            <div className="bg-slate-800 p-4 rounded-lg">
              {feeds.length === 0 ? (<div className="text-slate-400">No feeds found</div>) : (<ul className="space-y-1">
                  {feeds.map((feed) => (<li key={feed.id} className="flex justify-between">
                      <span>
                        <span className="text-blue-400">ID: {feed.id}</span> - 
                        <span className="text-white font-bold"> {feed.name || "[empty]"}</span>
                      </span>
                      <button className="text-xs bg-slate-700 px-2 py-1 rounded hover:bg-slate-600" onClick={() => setInspectFeed(feed.id)}>
                        Inspect
                      </button>
                    </li>))}
                </ul>)}
          ...
          {inspectedFeed && (<div className="mb-6">
              ...
              <div className="bg-slate-800 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  {Object.entries(inspectedFeed).map(([key, value]) => (<div key={key} className="contents">
                ...
                <button className="mt-4 text-xs bg-slate-700 px-2 py-1 rounded hover:bg-slate-600" onClick={() => setInspectFeed(null)}>
                  Close Inspector
                </button>
              </div>
            </div>)}
