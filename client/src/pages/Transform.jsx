...
export default function Transform() {
    ...
    const handleSubmit = (e) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const response = yield fetch('/api/transform/csv', {
                method: 'POST',
                body: formData,
            });
            clearInterval(progressInterval);
            if (!response.ok) {
                throw new Error(`Server responded with ${response.status}: ${yield response.text()}`);
            }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <Card className="border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            'border-slate-200 hover:border-slate-300 dark:border-slate-800 dark:hover:border-slate-700'}`} onClick={() => { var _a; return (_a = document.getElementById('fileInput')) === null || _a === void 0 ? void 0 : _a.click(); }}>
                    <input type="file" id="fileInput" accept=".csv" onChange={handleFileChange} className="hidden" disabled={uploading}/>
                ...
                {progress > 0 && progress < 100 && (<div className="space-y-2">
                ...
                <Button type="submit" className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700" disabled={!file || uploading}>
        ...
        <div>
          ...
          {result && result.success && (<Card className="border-green-200 dark:border-green-800 shadow-sm mt-4">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">Transformation Successful</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2 pt-0">
                {result.input_rows && result.output_rows && (<p className="text-sm text-slate-600 dark:text-slate-400">
                    Transformed {result.input_rows} rows into {result.output_rows} rows
                  </p>)}
                {result.filename && (<p className="text-sm text-slate-600 dark:text-slate-400">
