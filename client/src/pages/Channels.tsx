...
import { Globe, RefreshCcw, Layers, Database, BarChart } from "lucide-react";
...
export default function Channels() {
  ...
  const toggleConnection = (id: string) => {
    setMarketplaces(prev => 
      prev.map(m => 
        m.id === id 
          ? { ...m, connected: !m.connected, lastSync: m.connected ? undefined : "Just now" } 
          : m
      )
    );
  };

  const toggleSync = (id: string, type: 'catalog' | 'inventory' | 'orders') => {
    setMarketplaces(prev => 
      prev.map(m => 
        m.id === id 
          ? { ...m, [type]: !m[type] } 
          ...
          <TabsContent value="connections">
            ...
            <div className="space-y-4">
              {marketplaces.map((marketplace) => (
                <Card key={marketplace.id} className="bg-slate-900 border-slate-800">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          <Switch 
                            id={`connection-${marketplace.id}`} 
                            checked={marketplace.connected}
                            onCheckedChange={() => toggleConnection(marketplace.id)}
                          />
                          <Label 
                            htmlFor={`connection-${marketplace.id}`}
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-800 grid grid-cols-3 gap-4">
                        <div className="flex items-start gap-3">
                          <Switch 
                            ...
                            checked={marketplace.catalog}
                            onCheckedChange={() => toggleSync(marketplace.id, 'catalog')}
                          />
                          <div>
                            <Label 
                              htmlFor={`catalog-${marketplace.id}`}
                              className="text-sm font-medium cursor-pointer"
                            >
                              Catalog Sync
                            </Label>
                        ...
                        <div className="flex items-start gap-3">
                          <Switch 
                            id={`inventory-${marketplace.id}`} 
                            checked={marketplace.inventory}
                            onCheckedChange={() => toggleSync(marketplace.id, 'inventory')}
                          />
                          <div>
                            <Label 
                              htmlFor={`inventory-${marketplace.id}`}
                              className="text-sm font-medium cursor-pointer"
                            >
                              Inventory Sync
                            </Label>
                        ...
                        <div className="flex items-start gap-3">
                          <Switch 
                            id={`orders-${marketplace.id}`} 
                            checked={marketplace.orders}
                            onCheckedChange={() => toggleSync(marketplace.id, 'orders')}
                          />
                          <div>
                            <Label 
                              htmlFor={`orders-${marketplace.id}`}
                              className="text-sm font-medium cursor-pointer"
                            >
                              Orders Sync
                            </Label>
