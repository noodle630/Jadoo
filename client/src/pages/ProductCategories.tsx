...
export default function ProductCategories() {
  ...
  const buildCategoryTree = (
    ...
    return filtered.map(category => {
      ...
      return {
        ...category,
        level,
        children: buildCategoryTree(categories, category.id, level + 1, path),
  ...
  const flattenCategoryTree = (tree: CategoryNode[]): CategoryNode[] => {
    return tree.reduce<CategoryNode[]>((acc, node) => {
      acc.push(node);
