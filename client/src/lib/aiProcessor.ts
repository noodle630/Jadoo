/**
 * Interface representing a marketplace with transformation rules
 */
export interface Marketplace {
  ...
  fieldTransformations: Record<string, FieldTransformation>;
}

/**
 * Interface for field transformation rules
 */
export interface FieldTransformation {
  type: 'rename' | 'format' | 'combine' | 'split' | 'standardize';
 ...
 */
function findMatchingKey(obj: Record<string, any>, target: string): string | null {
  ...
  for (const key of Object.keys(obj)) {
    ...
    if (
      (target === 'title' && (key === 'name' || key === 'productName')) ||
...
function generateFieldValue(
  ...
  switch (field) {
    ...
    case 'productName':
      ...
      if (item.sku) return `Product ${item.sku}`;
      return 'New Product';
      
    case 'description':
    case 'shortDescription':
      // Try to generate from title
      if (item.title) return `${item.title}. High quality product.`;
      if (item.name) return `${item.name}. High quality product.`;
 ...
 */
function optimizeTitle(title: string, marketplaceConfig: Marketplace): string {
  if (!title) return '';
  ...
  if (titleTransformation.params?.capitalization === 'title-case') {
    optimized = toTitleCase(optimized);
  } else if (titleTransformation.params?.capitalization === 'sentence-case') {
    optimized = toSentenceCase(optimized);
  }
  
  // Enforce length limits
  const maxLength = titleTransformation.params?.maxLength || 150;
 ...
 */
function mapCategory(category: string, marketplaceConfig: Marketplace): string {
 ...
 */
function standardizePrice(price: any): string {
